...
class EcommerceTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        #The app uses Token authentification
        self.token = Token.objects.get(user = self.user)
        self.client = APIClient()

        #we pass the token in all calls in the API
    ...
    def test_get_one_item(self):
        '''
        test ItemViewSet retrieve method 
        '''
    ...
    def test_order_is_less_than_stock(self):
        ...
        for i in self.items:
            current_stock = i.stock
            self.assertEqual(i.check_stock(current_stock-1),True)

    def test_create_order_with_more_than_stock(self):
        '''
        test OrderViewSet create method when order.quantity > item.stock
        '''
        for i in self.items :
            stock = i.stock 
    ...
    def test_create_order_with_equal_stock(self):
        '''
      s  test OrdersViewSet create method when order.quantity == item.stock
        '''
        for i in self.items:
            stock = i.stock
